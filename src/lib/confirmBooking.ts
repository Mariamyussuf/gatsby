...
export async function confirmBooking(
  ...
  try {
    console.log("[BOOKING] Starting confirmation:", pending.reference)
    ...
    if (existingTxnError) {
      console.error("[BOOKING] Failed to fetch transaction:", existingTxnError)
      throw existingTxnError
    }

    if (existingTxn?.payment_status === "confirmed") {
      console.log("[BOOKING] Transaction already confirmed")
    ...
    // Small delay helps avoid race conditions after redirect
    await new Promise((resolve) => setTimeout(resolve, 2000))

    console.log("[BOOKING] Checking table availability")

    // Verify table still has space
    const { data: freshTable, error: tableFetchError } = await supabase
      .from("gala_tables")
    ...
    if (tableFetchError) {
      console.error("[BOOKING] Table fetch failed:", tableFetchError)
      throw tableFetchError
    }

    if (
      freshTable &&
      freshTable.seats_booked + pending.quantity > freshTable.seats_total
    ) {
    ...
    if (txnError) {
      console.error("[BOOKING] Transaction update failed:", txnError)
      throw txnError
    }
    ...
    try {
      ...
      console.log("[BOOKING] Seat RPC update successful")
    } catch (rpcErr) {
      console.warn("[BOOKING] RPC failed, using fallback update:", rpcErr)

      const { error: fallbackError } = await supabase
        .from("gala_tables")
        .update({
          seats_booked:
            pending.tableSeatsBooked + pending.quantity,
        })
        .eq("id", pending.tableId)

      if (fallbackError) {
        console.error("[BOOKING] Fallback seat update failed:", fallbackError)
        throw fallbackError
      }

      console.log("[BOOKING] Fallback seat update successful")
    }
    ...
    for (const att of pending.attendees) {
      ...
      console.log("[BOOKING] Inserting attendee:", attendeePayload)
      ...
      console.log("[BOOKING] Attendee created:", att.email)

      // Create VVIP pickup if needed
      if (pending.tierName === "VVIP") {
        console.log("[BOOKING] Creating VVIP pickup")
        ...
        if (rec) {
          ...
          console.log("[BOOKING] VVIP pickup created")
        }
      }
    }
    ...
    setTimeout(async () => {
      try {
        console.log("[BOOKING] Triggering confirmation emails")
        ...
        console.log("[BOOKING] Email function status:", res.status)
        console.log("[BOOKING] Email function response:", text)

        if (!res.ok) {
          console.warn(
            "[BOOKING] Email sending failed but booking already confirmed"
          )
        }
      } catch (emailErr: any) {
        console.warn(
    ...
    console.log("[BOOKING] Booking confirmation completed successfully")
    ...
    console.error("[BOOKING] Fatal confirmation error:", err)
